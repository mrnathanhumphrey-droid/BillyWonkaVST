...
## Commits that landed this

- `d9d47bb` — Fix macOS bundle corruption in CI packaging and install.sh
  (cp -R → ditto, both in workflow and install_mac.sh)
- `4a785c7` — Skip AU format on CI to avoid auval hang
  (BUILD_AU=OFF CMake option, passed via CI configure)
- `04ae3b7` — Zip macOS bundles on the build runner before upload-artifact
  (workflow restructure)

First clean macOS release: **v3.0.6**.
...
```bash
codesign -dv --verbose=4 "/path/to/BW BASS.app"
```
